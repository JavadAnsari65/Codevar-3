...
function ChangeForm(Element, Type) {
    if (Type == 'Login') {
        Element.setAttribute('FormActive', 'Login')
        ContainerForm.classList.remove('ContainerFormCreateActive')
        ContainerForm.classList.add('ContainerFormLoginActive')
        setTimeout(function () {
            ContainerForm.classList.remove('ContainerFormCreateActiveHideFormLogin')
            ContainerForm.classList.add('ContainerFormLoginActiveHideFormCreate')
        }, 250)
    } else {
        ContainerForm.classList.add('ContainerFormCreateActive')
        ContainerForm.classList.remove('ContainerFormLoginActive')
        setTimeout(function () {
            ContainerForm.classList.add('ContainerFormCreateActiveHideFormLogin')
            ContainerForm.classList.remove('ContainerFormLoginActiveHideFormCreate')
        }, 250)
    }
}
...
function SubmitLogin() {
    ...
    SendAjax('/M/Login/LoginCheck', Data, 'POST', function (response) {
...
function SubmitCreate() {
    ...
    SendAjax('/M/CreateAccount', Data, 'POST', function (response) {
        let Status = response.Status
        if (Status == '200') {

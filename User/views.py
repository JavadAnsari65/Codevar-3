...
from ProjectGroupOrder.Tools import GetTimeIran, ValidationText, ValidationNumber , SerializerTool
...
def CreateAccount(request):
    ...
    if (ValidationText(UserName,4,65,True,True) and ValidationText(Email,0,100) and ValidationText(Password,7,100,True,True)):
        UserExists = User.objects.filter(UserName=UserName).first() or  MemberGroup.objects.filter(UserName=UserName).first()
        if UserExists == None:
...
def SubmitProject(request):
    ...
    if User is not None:
        ...
        if ValidationText(TitleProject,4,65) and ValidationText(Technologies,4,100) and ValidationText(DeadLine,0,5) and ValidationText(Budget,2,50) and ValidationText(Description,1,5000):
            if User.InfoIsComplete():
                Project.objects.create(User_id=User.id,Title=TitleProject,Technologies=Technologies,DeadLineByUser=DeadLine,BudgetByUser=Budget,Description=Description,DateSubmit=GetTimeIran(),StateProject='Checking')
                Context['Status'] = '200'
            else:
                Context['Status'] = '205'
        else:
...
def SubmitInfo(request):
    ...
    if User is not None:
        ...
        if ValidationText(UserNameAndFamily,3,150) and ValidationText(PhoneNumber,10,12) and ValidationText(Email,3,150):
    ...
    if User is not None:
        if ValidationText(ID):
            ...
            if ProjectExists is not None:
                ...
                Context['Object'] = SerializerTool(Project,[ProjectExists],'__All__',['TimePastSubmit','GetStateProject'])[0] or None
                Context['Status'] = '200'
            else:
                Context['Status'] = '204'
        else:
